...
For credential research, `telemetry.capture_login_passwords` deliberately records parsed SQL-auth passwords in the JSONL sink and requires `telemetry.stdout=false`. `payloads.capture_login_messages` retains every fully framed attacker-supplied TDS 4.2/5.0 LOGIN, LOGIN7, and stage-identified authentication continuation before parsing as a mode-`0600` artifact; truncated authentication continuations follow the same policy. Stored artifacts are SHA-256 content-addressed and deduplicated, while each capture event retains its own occurrence context and reports `duplicate=true` when the content was already present. Payload capture must be enabled. The older `capture_login7` key remains an alias for existing configurations. Treat both directories as sensitive evidence: restrict access, set retention, and keep them out of container logs and routine log shipping. Generic parser-failure telemetry remains credential-blind; direct-login framing failures expose at most the eight-byte TDS header needed to diagnose framing.
